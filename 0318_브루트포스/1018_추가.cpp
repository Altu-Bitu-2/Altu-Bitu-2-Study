...
int main() {
    int n, m; //행, 열의 수 입력받을 변수 선언
    int ans = SIZE + 1; //정답은 체스판 칸 개수 + 1 로 초기화

    //입력
    cin >> n >> m; //행, 열 입력받음
    vector<vector<char>> board(n, vector<char>(m)); //행: n, 열: m
    for (int i = 0; i < n; i++) { //보드 입력 (행에 대한 반복문)
        for (int j = 0; j < m; j++) { //열에 대한 반복문
            cin >> board[i][j]; //색 정보 입력받음
        }
    }

    //연산
    for (int i = 0; i <= n - 8; i++) { //i(행)에 대한 반복문
        for (int j = 0; j <= m - 8; j++) { //j(열)에 대한 반복문
